...
## 2024-05-25 - Caching ByteBuffers in Rendering Loops
**Learning:** In high-frequency rendering loops (e.g., 30fps animation decoding in ViewModels), avoid continuously allocating new wrapper objects like `ByteBuffer.wrap()`. Continuously doing this creates `HeapByteBuffer` objects per frame, which leads to memory garbage generation and GC pauses.
**Action:** Instead, cache and reuse a single `ByteBuffer` instance (e.g., re-wrap only if the underlying array instance changes) to eliminate `HeapByteBuffer` allocations, effectively reducing GC overhead.
